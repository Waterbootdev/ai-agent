...
def validated_path(path: Path, inside_working_directory : bool, directory: str) -> str|None:
...
def get_files_info(working_directory:str, directory: str|None=None) -> str:
    ...
    return '\n'.join(to_file_info(path.joinpath(name), name) for name in os.listdir(path.absolute())) 
