...
def response_function_calls_loop(
    verbose: bool,
    contens: List[Content],
    model: str,
    client: genai.Client,
...
def get_function_calls_or_raise_exception(
...
def append_function_calls(
    verbose: bool, contens: List[Content], function_calls: List[FunctionCall]
):
    try:
        contens.append(
            Content(role="tool", parts=call_functions(verbose, function_calls))
        )
